...
public class JellyfinService : IMediaProvider, IDisposable
{
    ...
    public JellyfinService(Microsoft.Extensions.Configuration.IConfiguration config)
    {
        ...
        try
        {
            _serverHostname = new Uri(_serverUrl).Host;
        }
        catch
        {
            _serverHostname = "Jellyfin";
        }
        _username = username;
        ...
        string authHeader = $"MediaBrowser Client=\"Baird Media Player\", Device=\"Baird Device\", DeviceId=\"{Guid.NewGuid()}\", Version=\"1.0.0\"";
    ...
    private async Task AuthenticateAsync(string username, string password)
    {
        ...
        string jsonBody = JsonSerializer.Serialize(authBody, AppJsonContext.Default.AuthRequest);
        ...
        if (root.TryGetProperty("User", out JsonElement userProp) && userProp.TryGetProperty("Id", out JsonElement idProp))
        {
        ...
        // Set default client headers for Jellyfin (required for Auth)
        string authHeader = $"MediaBrowser Client=\"Baird Media Player\", Device=\"Baird Device\", DeviceId=\"{Guid.NewGuid()}\", Version=\"1.0.0\"";
        string authHeaderWithToken = $"{authHeader}, Token=\"{_accessToken}\"";
    ...
    public async Task<IEnumerable<MediaItem>> GetListingAsync()
    {
        ...
        try
        {
            ...
            MovieQueryResult? result = JsonSerializer.Deserialize(json, AppJsonContext.Default.MovieQueryResult);
    ...
    public async Task<MediaItem?> GetItemAsync(string id)
    {
        ...
        try
        {
            string url = $"Users/{_userId}/Items/{id}?Fields=ProductionYear,RunTimeTicks,ProviderIds";
            HttpResponseMessage response = await _httpClient.GetAsync(url);
            ...
            string json = await response.Content.ReadAsStringAsync();
            MovieItem? item = JsonSerializer.Deserialize(json, AppJsonContext.Default.MovieItem);
    ...
    public async Task<IEnumerable<MediaItem>> GetChildrenAsync(string id)
    {
        ...
        try
        {
            ...
            string url = $"Users/{_userId}/Items?ParentId={itemId}&IncludeItemTypes=Season,Episode,Movie,Folder&Recursive=false&SortBy=ParentIndexNumber,IndexNumber,SortName&Fields=ProductionYear,ParentIndexNumber,RunTimeTicks";
    ...
    private MediaItem MapJellyfinItem(MovieItem m)
    {
        ...
        MediaType type = isContainer ? MediaType.Brand : MediaType.Video;
        ...
        TimeSpan duration = m.RunTimeTicks.HasValue
