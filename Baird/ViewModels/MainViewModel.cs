...
public class MainViewModel : ReactiveObject
{
    ...
    private string? GetNextSeasonId(string? currentSeasonId)
    {
        ...
        string[] parts = currentSeasonId.Split('|');
        if (parts.Length != 2)
        {
            return null;
        }
    ...
    private async Task<MediaItem?> TryLoadNextSeasonFirstEpisode()
    {
        ...
        string? nextSeasonId = GetNextSeasonId(_currentSeasonId);
        ...
        try
        {
            IEnumerable<MediaItem> nextSeasonEpisodes = await _dataService.GetChildrenAsync(nextSeasonId);
    ...
    public async Task PlayNextEpisodeOrGoBack()
    {
        ...
        else
        {
            ...
            MediaItem? nextSeasonEpisode = await TryLoadNextSeasonFirstEpisode();
    ...
    public void OpenProgramme(Baird.Services.MediaItem programme)
    {
        ...
        vm.PlayRequested += (s, item) =>
        {
            ...
            {
                string[] parts = programme.Id.Split('|');
