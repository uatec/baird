...
public class HistoryViewModel : ReactiveObject
{
    ...
    public async Task RefreshAsync()
    {
        ...
        IEnumerable<MediaItem> newItems = await _dataService.GetHistoryItemsAsync();
        ...
        foreach (MediaItem? item in toRemove)
        {
        ...
        int index = 0;
        foreach (MediaItem newItem in newItems)
        {
            ...
            MediaItem? existing = HistoryItems.FirstOrDefault(x => x.Id == newItem.Id);
            if (existing != null)
            {
