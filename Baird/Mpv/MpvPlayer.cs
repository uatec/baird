...
public class MpvPlayer : IDisposable
{
    ...
    private readonly System.Threading.Thread? _eventThread;
    ...
    public MpvPlayer()
    {
        ...
        int res = LibMpv.mpv_initialize(_mpvHandle);
        if (res < 0)
        {
    ...
    private void EventLoop()
    {
        ...
        while (_eventLoopRunning)
        {
            try
            {
                ...
                LibMpv.MpvEvent evt = Marshal.PtrToStructure<LibMpv.MpvEvent>(eventPtr);
                ...
                if (evt.EventId == LibMpv.MpvEventId.EndFile)
                {
                    ...
                    {
                        LibMpv.MpvEndFileEvent endFileEvent = Marshal.PtrToStructure<LibMpv.MpvEndFileEvent>(evt.Data);
    ...
    public void InitializeOpenGl(IntPtr procAddressCallback, Action requestRender)
    {
        ...
        if (res < 0)
        {
            throw new Exception($"Failed to create render context: {res}");
        }
    ...
    public void UpdateVideoStatus()
    {
        if (State == PlaybackState.Loading)
        {
            ...
            string? time = GetPropertyString("time-pos");
    ...
    public void Command(params string[] args)
    {
        ...
        string cmdString = string.Join(" ", args); // Simple join might generally work for simple args, but quoting is safer.
    ...
    {
        nint ptr = LibMpv.mpv_get_property_string(_mpvHandle, name);
        if (ptr == IntPtr.Zero)
        {
            return null;
        }
    ...
    public void LogAudioTracks()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (type == "audio")
            {
                string? id = GetPropertyString($"track-list/{i}/id");
                string? lang = GetPropertyString($"track-list/{i}/lang");
                string? title = GetPropertyString($"track-list/{i}/title");
                string? selected = GetPropertyString($"track-list/{i}/selected");
